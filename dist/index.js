function h(n) {
  n = { ...{
    title: "正在加载中...",
    mask: !0
  }, ...n };
  let i = "data:image/png;base64,";
  document.getElementById("vkcyan_show_loading") && o();
  const a = d();
  let r = s(i);
  a.appendChild(r), n.title != "" && a.appendChild(l()), document.body.appendChild(a), n.mask && document.body.appendChild(c());
  function o() {
    let e = document.getElementById("vkcyan_show_loading"), t = document.getElementById("vkcyan_show_loading_mask");
    e.remove(), t && t.remove();
  }
  function d() {
    const e = document.createElement("div");
    return e.id = "vkcyan_show_loading", e.style.cssText = `
    position:fixed;
    z-index:99999;
    left:50%;
    top:50%;
    transform:translate(-50%, -50%);
    background-color:rgba(17,17,17,0.7);
    border-radius:8px;
    display:flex;
    flex-direction:column;
    align-items:center;
    padding:16px 20px;
    min-width: 104px;
    `, e;
  }
  function l() {
    const e = document.createElement("div");
    return e.style.cssText = "color:white;font-size:15px;max-width:200px;text-align:center", e.innerText = n.title, e;
  }
  function c() {
    let e = document.createElement("div");
    return e.id = "vkcyan_show_loading_mask", e.style.cssText = `
    position:fixed;
    z-index:99998;
    width:100vw;
    height:100vh;
    background-color:rgba(0,0,0,0);
    top:0;
    left:0;
    `, e;
  }
  function s(e) {
    let t = document.createElement("img");
    return t.src = e, t.style.cssText = "width:56px;height:56px;margin-left:16px;margin-right:16px;", n.title != "" && (t.style.marginBottom = "10px"), t.animate(
      [
        {
          transform: "rotate(0deg)"
        },
        {
          transform: "rotate(180deg)"
        },
        {
          transform: "rotate(360deg)"
        }
      ],
      {
        delay: 0,
        duration: 1e3,
        iterations: 1 / 0,
        fill: "forwards"
      }
    ), t;
  }
  return {
    close: o
  };
}
export {
  h as showLoading
};
